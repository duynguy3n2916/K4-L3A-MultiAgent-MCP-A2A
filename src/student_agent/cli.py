...
async def _run(root: Path, clean: bool = False) -> None:
    ...
    if not remaining_case_ids:
        print("All 100 cases already processed.")
        return
        ...
        try:
            async with connect_gateway(
                ...
                while remaining_case_ids:
                    ...
                    print(f"Solved {case_id} ({100 - len(remaining_case_ids)}/100)")
...
def parser() -> argparse.ArgumentParser:
    ...
    run_cmd.add_argument(
        "--clean",
        action="store_true",
        help="clean previous outputs and traces before running",
    )
...
def main() -> None:
    ...
    try:
        if args.command == "validate-inputs":
            ...
            asyncio.run(_run(root, clean=getattr(args, "clean", False)))
